...
(function() {
  ...
  function stopGame() {
    ...
    // 4b. Если был прямой заход на /#game — снимаем флаг:
    //     возвращаем фейд оверлею, видимость сцены и нормальный
    //     bg body. Делаем это ДО remove('visible'), чтобы fade-out
    //     оверлея сработал, а сцена была видна для отъезда камеры.
    document.documentElement.classList.remove('hash-game');

    // 4c. Сначала fade-out оверлея (0.4s),
